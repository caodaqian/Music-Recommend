...
var field_userID = document.getElementById('userID');
...
ajax_get("/api/issuper/" + "?uid=" + userID, function (data) {
    ...
    field_userID.value = userID;
    field_userName.value = userName;
})

// 接收json歌曲数据
var t = document.getElementById('t_recommend');
ajax_get("/api/getmusicbase/", function (data) {
    var tmp = JSON.parse(data);
    for (var i = 0; i < tmp.length; i++){
        var tr = document.createElement('tr');
        var songName = tmp[i][0];
        var songArtist = tmp[i][1];
        var songID = tmp[i][2];
